...
// CHECK-NEXT:     %[[READ_FROM_FIFO_0:.*]] = ktdf.read_from_fifo %[[FIFO_0]] : <"l1lu-to-sfu", 64xf16> -> tensor<64xf16>
// CHECK-NEXT:     %[[FIFO_1:.*]] = ktdf.fifo.allocate() -> !ktdf.fifo.slot<"sfu-to-l1su", 8xf32>
// CHECK-NEXT:     %[[READ_FROM_FIFO_1:.*]] = ktdf.read_from_fifo %[[FIFO_1]] : <"sfu-to-l1su", 8xf32> -> tensor<8xf32>
...
module {
  func.func @read_from_fifo_static() {
    ...
    %slot0 = ktdf.fifo.allocate() -> !ktdf.fifo.slot<"l1lu-to-sfu", 64xf16>
    
    // Read from the FIFO slot into a tensor
    %data0 = ktdf.read_from_fifo %slot0 : !ktdf.fifo.slot<"l1lu-to-sfu", 64xf16> -> tensor<64xf16>
    
    // Allocate another FIFO slot with 8 f32 elements
    %slot1 = ktdf.fifo.allocate() -> !ktdf.fifo.slot<"sfu-to-l1su", 8xf32>
    
    // Read from the second FIFO slot
    %data1 = ktdf.read_from_fifo %slot1 : !ktdf.fifo.slot<"sfu-to-l1su", 8xf32> -> tensor<8xf32>
